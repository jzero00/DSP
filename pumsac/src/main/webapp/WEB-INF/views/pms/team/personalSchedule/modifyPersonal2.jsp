				...
				<div class="card-body pad">
					<form id="modifyForm" name="modifyForm" action="modify?${_csrf.parameterName}=${_csrf.token}" role="form" method="post">
						...
						<div class="row">
							...
							<div class="form-group col-sm-4">
								<label>최종승인여부</label>
								<select name="approvalYn" class="form-control" ${sessionScope.loginUser.role eq 'ROLE_ADMIN' ? '' : 'disabled'}>
									<option value="n" ${detailPersonal.approvalYn eq 'n' ? 'selected' : ''}>거부</option>
									<option value="y" ${detailPersonal.approvalYn eq 'y' ? 'selected' : ''}>승인</option>
								</select>
							</div>
						</div>
